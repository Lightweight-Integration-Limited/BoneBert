...
The code is based on NER training code in BlueBERT (https://github.com/ncbi-nlp/bluebert).
Here, we replace the original DataProcessor for our tagging scheme.
@Author: Zhihao Dai
"""
        ...
        with tf.gfile.Open(input_file, "r") as f:
            ...
            for line in f:
                ...
                if len(contends) == 0:
                    ...
                    if len(words) > 100:
                        # split if the sentence is longer than 100
                        while len(words) > 100:
                            tmplabel = labels[:100]
...
'''
////////////////////////////////////////////////////////////
Here, we introduce two processors for first and second stages of fine-tuning.
////////////////////////////////////////////////////////////
'''
class FractureProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        l1 = self._read_data(os.path.join(data_dir, "train.bert"))
        # l2 = self._read_data(os.path.join(data_dir, "val.tsv"))
        # return self._create_example(l1 + l2, "train")
        return self._create_example(l1, "train")

    def get_dev_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "val.bert")), "dev")

    def get_test_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "test.bert")), "test")

    def get_labels(self):
        return [
        "B_POSITIVE", "B_NEGATIVE", "B_UNCERTAIN", "B_IGNORED",
        "B_TYPE", "B_BONE", "B_BONE_PART",
         "I", "O", "X", "[CLS]", "[SEP]"
         ]
...
class ExtraProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        l1 = self._read_data(os.path.join(data_dir, "extra.bert"))
        # l2 = self._read_data(os.path.join(data_dir, "val.tsv"))
        # return self._create_example(l1 + l2, "train")
        return self._create_example(l1, "train")
...
def main(_):
    ...
    processors = {
        ...
        "fracture": FractureProcessor,
        "extra": ExtraProcessor,
    }
